...
- **불성실 참여 탐지 (베타)** — 길드 전체 변동을 상쇄하고 개인만 이탈한 경우를 탐지 (아래 참조), 누적 탐지 횟수 TOP 10
- **스펙 다운 감지** — 장비 환산 점수 하락으로 설명되는 건을 분리하고, 인정 시 개인 baseline 갱신
- **단체 하락 알림** — 100명 이상 동시 하락 시 관리자에게 통지 (판정은 자동 보정되므로 조치 불필요)
...
**baseline** — 회원별 `spec_down_from_week` 이전 데이터는 판정에서 제외되지만 히스토리 표시에는 남는다. 누적 탐지 횟수는 인정 시점의 값을 `spec_down_prior_count`에 동결해 두고 이후 발생분만 더해 이력을 보존한다.

baseline 은 **개인 단위로만** 설정한다. 길드 전체가 함께 떨어지는 상황은 탐지의 환경 보정이 매주 자동으로 상쇄하므로, 과거 주차를 잘라낼 이유가 없다. 실측상 길드 단위 baseline 을 설정하면 누적 탐지 이력이 31명 42건에서 6명 6건으로 줄지만, 그 대가로 얻는 개인 평균 변화는 중앙값 `0.0%`이며 어느 화면에도 노출되지 않는 값이다. `guild_baseline_from_week` 읽기 경로는 남아 있으나 설정 수단은 제공하지 않는다.
...
| 경로 | 설명 | 인증 |
|---|---|---|
| `/` | 개요 (통계·차트·평균 등수/상승폭 TOP 10·전체 길드원) | 공개 |
| `/static/member.html?name=<닉>` | 회원 히스토리 | 공개 |
| `/admin` | 관리자 개요 (통계·차트) | 필요 |
| `/manage` | 리더보드 4종 TOP 10 · 벌금 · 탈퇴 등 관리 작업 | 필요 |
| `/detection` | 불성실 참여 탐지 (베타) · 누적 TOP 10 · 스펙 다운 인정 | 필요 |

관리자 페이지는 HTML 자체는 공개로 서빙되고, 진입 시 `/api/auth-status`로 판정해
비로그인이면 로그인 페이지로 보낸다. 데이터를 내려주는 API가 서버에서 401로 막으므로
실질적인 노출은 없다.
...
create_table.py         # 최초 1회 — DynamoDB 테이블 생성
